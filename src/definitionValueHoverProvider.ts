...
import { definitionReferenceAt, findDefineAssignments, activeAssignment, DefineAssignment } from './definitionValues';
import { containerBoundsAt } from './definitionIndex';

function describeAssignment(name: string, assignment: DefineAssignment, total: number): vscode.MarkdownString {
...
export class DefinitionValueHoverProvider implements vscode.HoverProvider {
    provideHover(document: vscode.TextDocument, position: vscode.Position): vscode.Hover | undefined {
        ...
        const bounds = containerBoundsAt(text.replace(/\r/g, '').split('\n'), position.line);
        const found = findDefineAssignments(text, ref.name, bounds);
        ...
        const active = activeAssignment(found, position.line)!;
