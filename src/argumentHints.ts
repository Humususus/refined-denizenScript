...
/** One argument of a command's documented syntax. */
export interface SyntaxParameter {
    /** The token exactly as the syntax writes it, e.g. `(targets:<player>|...)`. */
    text: string;
    /** The `prefix` of a `prefix:value` argument, lowercased, or null when the argument is bare. */
    prefix: string | null;
    /** Whether the syntax marks it required -- `[...]` rather than `(...)`. */
    required: boolean;
}
 ...
 * those apart.
 */
export function parseSyntaxParameter(text: string): SyntaxParameter {
 ...
 */
export function splitWrittenArguments(commandLine: string): string[] {
 ...
 */
export function remainingArguments(parameters: SyntaxParameter[], commandLine: string): SyntaxParameter[] {
    ...
    const remaining: SyntaxParameter[] = [];
 ...
 */
export function hintTextFor(parameters: SyntaxParameter[], commandLine: string): string | null {
