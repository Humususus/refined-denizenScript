...
/** One implicitly-available definition, with why it exists so the caller can say so. */
export interface ScopeDefinition {
    /** The name as written inside `<[...]>`. */
    name: string;
    /** Short human-readable origin, e.g. "foreach loop" — shown as the completion's detail. */
    source: string;
}

/** Tag parts that put per-entry definitions in scope, and what those are. */
const TAG_SCOPES: ReadonlyMap<string, string[]> = new Map([
...
function argumentValue(commandLine: string, prefix: string): string | null {
 ...
 */
export function loopDefinitions(commandLine: string): ScopeDefinition[] {
    ...
    if (command !== 'while') {
        definitions.push({ name: argumentValue(rest, 'as') ?? 'value', source });
    }
    if (command !== 'repeat') {
    ...
    if (command === 'foreach') {
        definitions.push({ name: argumentValue(rest, 'key') ?? 'key', source });
    }
    return definitions;
}
 ...
 */
export function enclosingLoopDefinitions(lines: string[], line: number): ScopeDefinition[] {
    const definitions: ScopeDefinition[] = [];
 ...
 */
export function tagScopeDefinitions(lineText: string, character: number): ScopeDefinition[] {
    const definitions: ScopeDefinition[] = [];
...
export function definitionsInScope(lines: string[], line: number, character: number): ScopeDefinition[] {
    const fromTags = tagScopeDefinitions(lines[line] ?? '', character);
