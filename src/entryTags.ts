...
export const ENTRY_TAGS_BY_COMMAND : Map<string, string[]> = new Map<string, string[]>([
 ...
 */
export function findSaveEntries(lines: string[], uptoLine: number): SaveEntry[] {
    const found: SaveEntry[] = [];
 ...
 */
export function entryTagsFor(entryName: string, saved: SaveEntry[]): string[] {
    const match = saved.find(e => e.name === entryName.toLowerCase());
    if (match !== undefined) {
        const tags = ENTRY_TAGS_BY_COMMAND.get(match.command);
        if (tags !== undefined) {
            return tags;
        }
    }
    return ALL_ENTRY_TAG_NAMES;
}
