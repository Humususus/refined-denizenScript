...
import { hintTextFor, parseSyntaxParameter, SyntaxParameter } from './argumentHints';

/** Reads the parameter texts out of a signature help response. */
function parametersOf(help: vscode.SignatureHelp | undefined): SyntaxParameter[] {
    ...
    return texts.map(parseSyntaxParameter);
}

export class ArgumentHintsProvider implements vscode.InlayHintsProvider {
     ...
     * any sooner).
     */
    constructor(private readonly usingTypeScriptServer: boolean) {}
    ...
    async provideInlayHints(document: vscode.TextDocument, range: vscode.Range): Promise<vscode.InlayHint[]> {
        if (!vscode.workspace.getConfiguration('denizenscript').get<boolean>('inlineArgumentHints', true)) {
            return [];
        }
        ...
        const help = await vscode.commands.executeCommand<vscode.SignatureHelp | undefined>(
            'vscode.executeSignatureHelpProvider', document.uri, new vscode.Position(position.line, lineText.length));
        const parameters = parametersOf(help);
        if (parameters.length === 0) {
            return [];
        }
        const text = hintTextFor(parameters, lineText.slice(0, position.character));
        if (text === null) {
            return [];
        }
...
export function activateArgumentHints(context: vscode.ExtensionContext, usingTypeScriptServer: boolean): void {
