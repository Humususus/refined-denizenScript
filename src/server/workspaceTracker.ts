...
import { ScriptingWorkspaceData } from './checker/containerConvert';
import type { MetaDocs } from './metaDocs/metaTypes';
import type { ExtraData } from './metaDocs/extraData';

/** How the tracker is told to build a checker, so the caller keeps ownership of the meta. */
export interface CheckerFactory {
    meta: MetaDocs | null;
 ...
 */
export function findScriptFiles(root: string): string[] {
    const found: string[] = [];
...
/** One tracked file: the checker that produced its data, and the path its containers are labelled with. */
...
export class WorkspaceTracker {
    ...
    readonly files: Map<string, TrackedFile> = new Map();

    /** The merged data of every tracked file, or null before the first scan. (:18, `WorkspaceData`) */
    workspaceData: ScriptingWorkspaceData | null = null;
     ...
     */
    private add(filePath: string, checker: ScriptChecker): void {
    ...
    private mergeAll(): ScriptingWorkspaceData {
        const combined = new ScriptingWorkspaceData();
     ...
     */
    replace(filePath: string, checker: ScriptChecker): void {
     ...
     */
    firstScan(context: CheckerFactory): Map<string, ScriptChecker> {
        if (!this.enabled || this.root === null) {
            return new Map();
        }
     ...
     */
    scanPaths(paths: string[], context: CheckerFactory): Map<string, ScriptChecker> {
        const results = new Map<string, ScriptChecker>();
     ...
     */
    private runOne(filePath: string, context: CheckerFactory, surrounding: ScriptingWorkspaceData | null): ScriptChecker | null {
        let text: string;
        try {
    ...
    dataFor(): ScriptingWorkspaceData | null {
        if (!this.enabled) {
            return null;
        }
