 ...
 *
 * The C# stores sections as `Dictionary<LineTrackedString, object>` and leans on
 * `LineTrackedString`'s text-only `Equals`/`GetHashCode` override (ScriptChecker.cs:1376-1385)
 * for both lookup and duplicate detection. TypeScript `Map` keys compare by identity and cannot
 * be taught otherwise, so a section here is a `Map<string, SectionEntry>` keyed on
 * `LineTrackedString.textKey(...)` -- see the long note on that helper in ./scriptWarnings.
 *
 * That means the key's ORIGIN (its line and column) has to be carried in the value, because
 * later phases need it: ScriptChecker.cs:1697 reports `invalid_container` at `title.Line`, and
 * Phase 2C-3 consumes exactly that. Hence the pair shape rather than a bare value.
 *
 * `key` is `readonly` on purpose: C#'s `dict[k] = v` on an ALREADY-PRESENT key replaces the
 * value and keeps the original key object. `setEntry` below reproduces that, and marking `key`
 * readonly stops a future edit from quietly overwriting the first-seen position -- which the
 * duplicate_key/duplicate_script paths depend on.
 */
export interface SectionEntry {
    /** The key as it was read, with its line and column. */
    readonly key: LineTrackedString;
    /** The value, replaced in place when the same key text is assigned again. */
    value: SectionValue;
}

/**
 * A script section: an ordered map from key text to its entry. Insertion order is load-bearing
 * -- ScriptChecker.cs:1590 reads `sec.Keys.Last()`.
 */
export type ScriptSection = Map<string, SectionEntry>;

/**
 * What a section key can hold. The C# types this as `object` and the three shapes below are
 * exactly the three things it ever assigns: a nested section (:1618), a list (:1477), or a
 * scalar (:1633). Modelling it as a three-way union rather than `unknown`/`any` means consumers
 * must discriminate, and all three arms are distinguishable at runtime without a tag field
 * (`instanceof LineTrackedString`, `Array.isArray`, `instanceof Map`).
 */
export type SectionValue = ScriptSection | ScriptList | LineTrackedString;

/** A list of commands/values under a key. */
export type ScriptList = ScriptListEntry[];

/**
 * What a list can hold: a plain entry (:1534, :1556), or a single-key section wrapping a
 * command's own sub-list (:1487).
 */
export type ScriptListEntry = LineTrackedString | ScriptSection;
 ...
 */
export const COMMANDS_WITH_COLONS_AND_ARGUMENTS = new Set<string>(['if', 'else', 'foreach', 'while', 'repeat', 'choose', 'case']);

/** ScriptChecker.cs:57-60 (`CommandsWithColonsButNoArguments`). See the note above on the export. */
export const COMMANDS_WITH_COLONS_BUT_NO_ARGUMENTS = new Set<string>(['else', 'default', 'random']);
 ...
 */
function setEntry(section: ScriptSection, key: LineTrackedString, value: SectionValue): void {
    const mapKey = LineTrackedString.textKey(key);
 ...
 */
export function canWarnAboutCommandMissingDash(args: string[], currentRootSection: ScriptSection | null): boolean {
    ...
    const typeEntry = currentRootSection.get(LineTrackedString.textKey('type'));
    ...
    if (!(typeValue instanceof LineTrackedString)) {
        return true;
    }
 ...
 */
export function gatherActualContainers(checker: ScriptChecker): ScriptSection {
    ...
    const rootScriptSection: ScriptSection = new Map<string, SectionEntry>();
    const spacedsections = new Map<number, ScriptSection>([[0, rootScriptSection]]);
    const spacedlists = new Map<number, ScriptList>();
    let currentSection: ScriptSection = rootScriptSection;
    let currentRootSection: ScriptSection | null = null;
    let pspaces = 0;
    let secwaiting: LineTrackedString | null = null;
    let clist: ScriptList | null = null;
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const spaces = countPreSpaces(line);
        ...
        if (spaces < pspaces) {
            ...
            if (tempList !== undefined) {
                ...
                checker.warn(
                    ...
                    i,
                    'shrunk_spacing',
                    `Simple spacing error - shrunk unexpectedly to new space count, from ${pspaces} down to ${spaces}, while expecting any of: ${Array.from(spacedsections.keys()).join(', ')}.`,
                    0,
                    spaces
                );
                pspaces = spaces;
        ...
        if (cleaned.startsWith('- ')) {
            ...
            if (secwaiting !== null) {
                if (clist === null) {
                    ...
                    if (currentSection.has(LineTrackedString.textKey(secwaiting))) {
                        checker.warnAt(
                            checker.errors,
                            'duplicate_key',
                    ...
                    if (spaces <= pspaces) {
                        ...
                        checker.warnAt(
                            checker.errors,
                            'empty_command_section',
                            'Script section within command is empty (add contents, or remove the section).',
                            secwaiting
                        );
                    }
                    // ScriptChecker.cs:1486-1491
                    const newclist: ScriptList = [];
                    const subSection: ScriptSection = new Map<string, SectionEntry>();
                    ...
                    checker.warn(
                        checker.warnings,
                        i,
                        'growing_spacing_impossible',
                        "Line grew when that isn't possible (spacing error?).",
                        0,
                        spaces
                    );
                    pspaces = spaces;
            ...
            if (textLow.endsWith(':')) {
                if (textLow.startsWith('definemap ')) {
                    ...
                    clist.push(new LineTrackedString(i, textRaw, cleanStartCut + 2));
                    while (i + 1 < lines.length) {
                        const subLine = lines[i + 1].replaceAll('\t', '    ');
                        if (isNullOrWhiteSpace(subLine) || countPreSpaces(subLine) > spaces) {
                    ...
                    secwaiting = new LineTrackedString(i, textRaw.slice(0, -1), cleanStartCut + 2);
        ...
        if (spaces > pspaces) {
            ...
            const sect: ScriptSection = new Map<string, SectionEntry>();
        ...
        if (endofline.length === 0) {
            ...
            secwaiting = new LineTrackedString(i, toLowerFast(startofline), cleanStartCut);
        } else {
            ...
            setEntry(
                currentSection,
                new LineTrackedString(i, toLowerFast(startofline), cleanStartCut),
                new LineTrackedString(i, endofline, endIndex)
            );
        }
 ...
 */
function anyOpenSectionEndsInData(spacedsections: Map<number, ScriptSection>): boolean {
