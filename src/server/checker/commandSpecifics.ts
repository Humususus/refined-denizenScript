...
import { isAsyncSafeCommand } from './asyncSafety';
 ...
 */
export function checkSingleCommand(
    checker: ScriptChecker,
    line: number,
    startChar: number,
    commandText: string,
    ...
    if (commandText.includes('@')) {
        const range = containsObjectNotation(commandText);
        if (range !== null) {
            checker.warn(checker.warnings, line, 'raw_object_notation',
                ...
                startChar + range.start, startChar + range.end);
        }
    }
    ...
    const args: CommandArgument[] = argumentText === null
        ? []
        : buildArgs(line, startChar + rawName.length + 1, argumentText, checker);
    ...
    if (command === undefined) {
        ...
        if (commandName !== 'case' && commandName !== 'default') {
            checker.warn(checker.errors, line, 'unknown_command',
                `Unknown command \`${commandName.replaceAll('`', "'")}\` (typo? Use \`!command [...]\` to find a valid command).`,
                startChar, startChar + cmdLen);
        }
        return;
    }
    ...
    if (!isNullOrWhiteSpace(command.deprecated)) {
        checker.warn(checker.errors, line, 'deprecated_command',
            `Command '${command.name}' is deprecated: ${command.deprecated}`, startChar, startChar + cmdLen);
    }
    ...
    if (argCount < command.required) {
        checker.warn(checker.errors, line, 'too_few_args',
            `Insufficient arguments... the \`${command.name}\` command requires at least ${command.required} arguments, but you only provided ${argCount}.`,
            startChar, startChar + commandText.length);
    }
    if (argCount > command.maximum) {
        checker.warn(checker.errors, line, 'too_many_args',
            `Too many arguments... the \`${command.name}\` command requires no more than ${command.maximum} arguments, but you provided ${argCount}. Did you forget 'quotes'?`,
            startChar, startChar + commandText.length);
    }
    ...
    if (context.insideAsyncBlock && !isAsyncSafeCommand(commandName)) {
        checker.warn(checker.warnings, line, 'async_unsafe_command',
            `The \`${command.name}\` command is not async-safe, so inside an \`async\` block it is handed to the main thread and the async queue waits on it. Move it outside the \`async:\` block, or drop the block if the whole task needs the main thread.`,
            startChar, startChar + cmdLen);
    }
    // ScriptChecker.cs:865-868
    const specific = COMMAND_CHECKERS.get(commandName);
    if (specific !== undefined) {
        specific(details);
    }
    // ScriptChecker.cs:869-877. Read from every argument, not from the filtered set -- `save:`
    // is one of the four the filter drops.
    const saveArgument = args.find(s => s.text.startsWith('save:'))?.text;
    ...
    for (const argument of args) {
        checkSingleArgument(checker, line, argument.startChar, argument.text, context, true, (l, s, t, c) => checkSingleTag(checker, l, s, t, c));
    }
}
