...
import { isAdvancedMatchable, createMatcher } from './advancedMatcher';
import { before } from './frenetic';
import type { ExtraData } from '../metaDocs/extraData';

/**
 * A could-matcher argument validator: `(word, precise) => confidence`.
 * (ExtraData.cs:339, `Func<string, bool, int>`)
 *
 * `precise` is the caller's demand for proof. False means "does this look vaguely right?", used
 * when scanning for a best match. True means "verify it", used when the answer will be reported.
 */
export type Validator = (word: string, precise: boolean) => number;

/** Known always-valid entity labels. (ExtraData.cs:151-154) */
export const SPECIAL_ENTITY_MATCHABLES = new Set<string>([
...
export const ITEM_COULD_MATCH_PREFIXES = new Set<string>([
 ...
 */
export const INVENTORY_MATCHERS = new Set<string>([
 ...
 */
function fallbackScore(word: string, precise: boolean, own: Iterable<string> | null, foreign: Set<string>[]): number {
    if (precise) {
        if (isAdvancedMatchable(word)) {
            if (own === null) {
...
export function matchEntity(data: ExtraData, word: string, precise: boolean): number {
    if (word.startsWith('entity_flagged:') || word.startsWith('player_flagged:') || word.startsWith('npc_flagged:')
        || SPECIAL_ENTITY_MATCHABLES.has(word)
...
export function matchItem(data: ExtraData, word: string, precise: boolean): number {
    ...
    if (ITEM_COULD_MATCH_PREFIXES.has(before(word, ':'))
...
export function matchInventory(data: ExtraData, word: string, precise: boolean): number {
    if (INVENTORY_MATCHERS.has(word)
        || word.startsWith('inventory_flagged:')) {
        return 10;
    }
    return fallbackScore(word, precise, INVENTORY_MATCHERS, [data.blocks, data.items, data.entities]);
}

/** Type matcher for blocks. (ExtraData.cs:264-292) */
export function matchBlock(data: ExtraData, word: string, precise: boolean): number {
 ...
 */
export function matchMaterial(data: ExtraData, word: string, precise: boolean): number {
    return Math.max(matchBlock(data, word, precise), matchItem(data, word, precise));
}

/** Type matcher for areas. (ExtraData.cs:301-321) */
export function matchArea(data: ExtraData, word: string, precise: boolean): number {
 ...
 */
export function matchWorld(_data: ExtraData, _word: string, _precise: boolean): number {
    return 1;
}
 ...
 */
export function knownValidatorTypes(data: ExtraData): Map<string, Validator> {
