...
import type { Validator } from './eventValidators';

/**
 * Helper to generate automatic logic for ScriptEvent#couldMatch.
 * (ScriptEventCouldMatcher.cs:11-144)
 */
export class ScriptEventCouldMatcher {
    /** The raw format string used to construct this couldMatcher. (:14) */
    format: string;

    /** The raw parts list used to construct this couldMatcher. (:17) */
    parts: string[];

    /**
     * One validator per word of the format. (:20)
     *
     * The path length must equal this length for a full match, and each word must satisfy the
     * validator at the same index.
     */
    validators: Validator[];

    /**
     * Argument indices controlling TEST ORDER, cheapest first. (:23)
     *
     * Built as two lists and concatenated: plain words and `a|b` alternations first, since those
     * are a string compare or a set lookup, then the `<type>` fill-ins, which run a whole
     * type matcher. `tryMatch` bails on the first zero, so ordering the cheap certain-failures
     * first is what keeps matching every script event line against every documented event
     * affordable. It is NOT a behaviour change -- see `tryMatch` on why the order cannot alter
     * the result.
     */
    argOrder: number[];
     ...
     * whole meta load.
     */
    constructor(format: string, error: (message: string) => void, knownValidatorTypes: Map<string, Validator>) {
        this.format = format;
        this.parts = format.split(' ');
        const validatorList: Validator[] = [];
        const argOrderList: number[] = [];
        const secondaryArgList: number[] = [];
        let index = 0;
        for (const arg of this.parts) {
            ...
            if (arg.startsWith('<')) {
                ...
                // :74-79
                const rawValues = new Set<string>(arg.split('|'));
     ...
     */
    tryMatch(pathBaseParts: string[], allowPartial: boolean, precise: boolean): number {
        // :98-104
     ...
     */
    isBetterMatchThan(pathBaseParts: string[], precise: boolean, matcher2: ScriptEventCouldMatcher): boolean {
 ...
 */
export function parseMatchers(format: string, validatorTypes: Map<string, Validator>, error: (message: string) => void): ScriptEventCouldMatcher[] {
    const matcherList: ScriptEventCouldMatcher[] = [];
 ...
 */
function buildMainContent(output: ScriptEventCouldMatcher[], format: string, validatorTypes: Map<string, Validator>, error: (message: string) => void): void {
