...
import { toLowerFast, before, beforeAndAfter } from './frenetic';
import { isInDataValueSet } from '../metaDocs/metaTypes';
...
/** A separated switch: its lowercased name and its raw, unfolded value. */
 ...
 */
export function separateSwitches(docs: MetaDocs, eventLine: string): { cleaned: string, switches: EventSwitch[] } {
    const parts = eventLine.split(' ');
    let output = '';
    const switches: EventSwitch[] = [];
    for (const part of parts) {
        if (part.includes(':') && !isInDataValueSet(docs, 'not_switches', before(part, ':')) && !isOnlyDigits(before(part, ':'))) {
            const [switchName, switchVal] = beforeAndAfter(part, ':');
            switches.push({ key: toLowerFast(switchName), value: switchVal });
        } else {
