 ...
 */
function bareEventText(line: string): string | null {
 ...
 */
export function matchEventLine(docs: MetaDocs, line: string): MetaEvent | null {
    const bare = bareEventText(line);
    if (bare === null) {
        return null;
    }
    const separated = separateSwitches(docs, bare);
    const parts = toLowerFast(separated.cleaned).split(' ');
    let best: MetaEvent | null = null;
    let bestMatcher: ScriptEventCouldMatcher | null = null;
 ...
 */
export function contextNamesForEvent(evt: MetaEvent): string[] {
