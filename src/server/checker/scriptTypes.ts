 ...
 *
 * The C# field initialisers ARE the defaults and they are not uniform: array fields default to
 * empty, `Strict` defaults to FALSE, and `CanHaveRandomScripts` defaults to TRUE. Six of the
 * sixteen entries omit `CanHaveRandomScripts` and rely on that true -- `procedure`, `task`,
 * `world`, `assignment`, `command`, `interact` -- and eleven are strict. `knownType()` below
 * applies all the defaults in one place so no entry can silently pick up the wrong one.
 */
export interface KnownScriptType {
    /** Keys that must always be present. (ScriptChecker.cs:888) */
    requiredKeys: string[];
    /** Keys that generally shouldn't be present unless something's gone wrong. (:891) */
    likelyBadKeys: string[];
    /** Value-based keys. (:894) */
    valueKeys: string[];
    /** Data list keys. (:897) */
    listKeys: string[];
    /** Script keys. (:900) */
    scriptKeys: string[];
    /** Whether unrecognised keys draw a warning. (:903) */
    strict: boolean;
    /** Whether this type can have random extra scripts attached. (:906) */
    canHaveRandomScripts: boolean;
}

/** Applies ScriptChecker.cs:888-906's field initialisers to a partial entry. */
function knownType(partial: Partial<KnownScriptType>): KnownScriptType {
    return {
        requiredKeys: partial.requiredKeys ?? [],
        likelyBadKeys: partial.likelyBadKeys ?? [],
        valueKeys: partial.valueKeys ?? [],
        listKeys: partial.listKeys ?? [],
        scriptKeys: partial.scriptKeys ?? [],
        strict: partial.strict ?? false,
        canHaveRandomScripts: partial.canHaveRandomScripts ?? true
    };
}
 ...
 */
export const KNOWN_SCRIPT_TYPES: Map<string, KnownScriptType> = new Map<string, KnownScriptType>([
...
export const ALWAYS_SCRIPT_KEYS: string[] = ['script', 'scripts', 'subscripts', 'subtasks', 'inject', 'injects', 'injectables', 'subprocedures'];

/** Keys that always mean simple data. (ScriptChecker.cs:48) */
export const ALWAYS_DATA_KEYS: string[] = ['data', 'description'];
 ...
 */
export function matchesSet(key: string, keySet: string[]): boolean {
