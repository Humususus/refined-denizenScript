...
/** One argument of a command line, with where it starts. Ported from ScriptChecker.cs:640-647. */
export interface CommandArgument {
    /** The character index that this argument starts at. (:643) */
    startChar: number;
    /** The text of the argument, with any wrapping quotes removed. (:646) */
    text: string;
}
 ...
 */
export function buildArgs(line: number, startChar: number, stringArgs: string, checker: ScriptChecker | null): CommandArgument[] {
    ...
    const matchList: CommandArgument[] = [];
                        ...
                        if (checker !== null) {
                            ...
                            if (!(hasSpace || (tagMarks !== 0 && matched.includes(' '))) && !matched.endsWith(':')) {
                                checker.warn(
                                    ...
                                    line,
                                    'bad_quotes',
                                    'Pointless quotes (arguments quoted but do not contain spaces).',
                                    startChar + start,
                                    startChar + i
                                );
                            }
