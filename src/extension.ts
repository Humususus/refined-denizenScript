...
type FlagCompletionKind = "player" | "server";
...
function isWorkspaceFlagCompletionContext(document: vscode.TextDocument, position: vscode.Position) : boolean {
    return getFlagCompletionKind(document, position) !== undefined;
}

function getFlagCompletionKind(document: vscode.TextDocument, position: vscode.Position) : FlagCompletionKind | undefined {
...
function getContainerTextUntilPosition(document: vscode.TextDocument, position: vscode.Position) : string {
    let startLine = 0;
    for (let line = position.line; line >= 0; line--) {
        if (isTopLevelContainerLine(document.lineAt(line).text)) {
            startLine = line;
...
function inferDefineValueFlagKind(value: string) : FlagCompletionKind | undefined {
...
function getContainerDefineTypes(document: vscode.TextDocument, position: vscode.Position) : Map<string, FlagCompletionKind> {
    const defineTypes = new Map<string, FlagCompletionKind>();
