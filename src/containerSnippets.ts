...
function body(...lines: [number, string][]): string {
    return lines.map(([level, text]) => ONE_LEVEL.repeat(level) + text).join('\n');
}

export interface ContainerSnippet {
    type: string;
    detail: string;
    /** The snippet body BELOW the container's name line, which the caller prepends. */
    body: string;
}

export const CONTAINER_SNIPPETS: ContainerSnippet[] = [
    // --- Denizen Core ---
    {
    ...
    },
    {
        ...
        body: body([1, 'type: world'], [1, 'debug: false'], [1, 'events:'],
            [2, 'on ${2:player joins}:'], [2, '- ${3:narrate "Hello there!"}'])
    },
    {
    ...
    // --- Denizen-Bukkit ---
    {
        ...
        body: body([1, 'type: assignment'], [1, 'debug: false'], [1, 'actions:'],
            [2, 'on ${2:assignment}:'], [2, '- ${3:narrate "Hello there!"}'])
    },
    {
        type: 'book', detail: 'Denizen book container',
        body: body([1, 'type: book'], [1, 'debug: false'], [1, 'title: ${2:My Book}'],
            [1, 'author: ${3:Author}'], [1, 'text:'], [1, '- ${4:Page one.}'])
    },
    {
        type: 'command', detail: 'Denizen command container',
        body: body([1, 'type: command'], [1, 'debug: false'], [1, 'name: ${2:mycommand}'],
            [1, 'description: ${3:My command.}'], [1, 'usage: /${2:mycommand}'],
            [1, 'script:'], [1, '- ${4:narrate "Hello there!"}'])
    },
    {
        type: 'economy', detail: 'Denizen economy container',
        body: body([1, 'type: economy'], [1, 'debug: false'], [1, 'priority: ${2:10}'],
            [1, 'name single: ${3:Coin}'], [1, 'name plural: ${4:Coins}'], [1, 'digits: ${5:2}'],
            [1, 'format: ${6:<[amount]> Coins}'], [1, 'balance: ${7:<player.flag[money]>}'],
            [1, 'has: ${8:<player.flag[money].is_more_than[<[amount]>]>}'],
            [1, 'withdraw:'], [1, '- ${9:flag player money:-:<[amount]>}'],
            [1, 'deposit:'], [1, '- ${10:flag player money:+:<[amount]>}'])
    },
    {
    ...
    },
    {
        ...
        body: body([1, 'type: interact'], [1, 'debug: false'], [1, 'steps:'],
            [2, '${2:1}:'], [3, '${3:click trigger}:'], [4, 'script:'], [4, '- ${4:narrate "Hello there!"}'])
    },
    {
    ...
    },
    {
        ...
        body: body([1, 'type: dialog'], [1, 'debug: false'], [1, 'base:'],
            [2, 'type: multi'], [2, 'title: <gray>${2:Добро пожаловать!}'], [2, 'columns: 1'],
            [1, 'bodies:'], [2, 'header:'], [3, 'type: message'],
            [3, 'message: <gray>${3:Введите отображаемое имя}'],
            [1, 'inputs:'], [2, '1:'], [3, 'type: text'], [3, 'label: ${4:Имя}'],
            [3, 'key: ${5:display_name}'],
            [1, 'buttons:'], [2, '1:'], [3, 'label: ${6:Подтвердить}'], [3, 'script:'],
            [3, '- define name <context.${5:display_name}>'], [3, '- narrate <[name]>'])
    }
];
 ...
 */
export function containerSnippetText(entry: ContainerSnippet): string {
