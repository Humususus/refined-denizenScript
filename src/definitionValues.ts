...
/** One `- define` line that plainly SETS `name`, and what it sets it to. */
export interface DefineAssignment {
    line: number;
    /** The value exactly as written -- everything after the name argument, unparsed. */
    value: string;
    /** Whether the line opens with `~define`, the off-thread form (see the command's own docs). */
    waitable: boolean;
}
