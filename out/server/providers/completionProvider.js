...
const PARAM_CANDIDATE_KINDS = {
    ...
    script: vscode_languageserver_1.CompletionItemKind.Method
};
/**
 ...
 */
function completeTagParameter(docs, extra, ctx, line, workspace, commandName = '') {
    ...
    const candidates = documented.tag.cleanName === 'map' && commandName === 'adjust'
        ? (0, tagParamCompleters_1.completeAdjustMapKeys)(docs, ctx.paramSoFar)
    ...
    for (const candidate of candidates) {
        ...
        if (candidate.kind === 'script' && candidate.script !== undefined) {
            ...
            item.documentation = (0, describe_1.describeScript)(candidate.script);
        }
 ...
 */
function lastTopLevelArgStart(trimmed) {
    let argStart = 0;
    let depth = 0;
 ...
 */
function completeTagAt(docs, extra, argThusFar, argStart, line, trace, workspace, commandName = '') {
...
function provideCompletions(docs, extra, text, offset, line, trace = true, workspace = null) {
    var _a;
