                ...
                if (listEntry instanceof scriptWarnings_1.LineTrackedString) {
                    ...
                    if (!onlyEntry.key.text.startsWith('definemap')) {
                        ...
                        if (checker.meta !== null && checker.meta.commands.has('async')
                            && (0, asyncSafety_1.asyncBlockCommandName)(onlyEntry.key.text) !== null) {
 ...
 */
function allSwitchesValid(docs, evt, switches) {
 ...
 */
function checkTypeSpecifics(checker, script, typeString, scriptSection, warnScript) {
    var _a, _b, _c, _d, _e;
    // ScriptChecker.cs:1146-1165
    if (typeString.text === 'command') {
        const nameString = (_a = scriptSection.get(scriptWarnings_1.LineTrackedString.textKey('name'))) === null || _a === void 0 ? void 0 : _a.value;
        if (nameString instanceof scriptWarnings_1.LineTrackedString) {
            const usageString = (_b = scriptSection.get(scriptWarnings_1.LineTrackedString.textKey('usage'))) === null || _b === void 0 ? void 0 : _b.value;
            if (usageString instanceof scriptWarnings_1.LineTrackedString) {
                ...
                if (!usageString.text.startsWith(`/${nameString.text} `) && usageString.text !== `/${nameString.text}`) {
                    warnScript(checker.minorWarnings, usageString.line, 'command_script_usage', "Command script usage key doesn't match the name key (the name is the actual thing you need to type in-game, the usage is for '/help' - refer to `!lang command script containers`)!");
                }
            }
            // :1157-1163
            const aliasList = (_c = scriptSection.get(scriptWarnings_1.LineTrackedString.textKey('aliases'))) === null || _c === void 0 ? void 0 : _c.value;
            if (Array.isArray(aliasList) && aliasList.length > 0) {
                const badAlias = aliasList.find(o => o instanceof scriptWarnings_1.LineTrackedString && o.text === nameString.text);
                if (badAlias instanceof scriptWarnings_1.LineTrackedString) {
                    warnScript(checker.warnings, badAlias.line, 'command_script_aliasname', "A command script alias should not be the same as the command script's name.");
                }
            }
        }
    }
    // ScriptChecker.cs:1166-1198
        ...
        const actionsMap = (_d = scriptSection.get(scriptWarnings_1.LineTrackedString.textKey('actions'))) === null || _d === void 0 ? void 0 : _d.value;
        if (actionsMap instanceof Map) {
            for (const entry of actionsMap.values()) {
                ...
                if (actionName.includes('@')) {
                    ...
                    checker.warn(checker.warnings, actionValue.line, 'action_object_notation', 'This action line appears to contain raw object notation. Object notation is not allowed in action lines.', start, end);
                }
                ...
                if (!checker.meta.actions.has(actionName)) {
                    ...
                    if (!exists) {
                        warnScript(checker.warnings, actionValue.line, 'action_missing', "Assignment script action listed doesn't exist. (Check `!act ...` to find proper action names)!");
                    }
                }
            }
        }
    ...
    // ScriptChecker.cs:1199-1319
        ...
        const eventsMap = (_e = scriptSection.get(scriptWarnings_1.LineTrackedString.textKey('events'))) === null || _e === void 0 ? void 0 : _e.value;
 ...
 */
function checkOneEventLine(checker, eventValue, warnScript) {
    // ScriptChecker.cs:1205. `on ` or `after ` is stripped; anything else keeps its whole text.
    let eventName = eventValue.text.substring(eventValue.text.startsWith('on ') ? 'on '.length : (eventValue.text.startsWith('after ') ? 'after '.length : 0));
    if (eventName.includes('@')) {
        const atRange = (0, tagChecks_1.containsObjectNotation)(eventName);
        if (atRange !== null) {
            ...
            checker.warn(checker.warnings, eventValue.line, 'event_object_notation', 'This event line appears to contain raw object notation. Object notation is not allowed in event lines.', start, end);
        }
    }
    ...
    // :1216-1217
    const separated = (0, eventTools_1.separateSwitches)(meta, eventName);
    ...
    if (matchedEvent === null) {
        ...
        if (matchedEvent === null) {
            warnScript(checker.warnings, eventValue.line, 'event_missing', "Script Event listed doesn't exist. (Check `!event ...` to find proper event lines)!");
        }
        else {
            ...
            warnScript(checker.warnings, eventValue.line, 'event_missing', `Script Event listed doesn't exist. Got partial match for '${matchedEvent.name}' - might be incomplete? Check documentation.`);
        }
        return;
    }
    ...
    for (const switchPair of switches) {
        if (switchPair.key === 'cancelled' || switchPair.key === 'ignorecancelled') {
            ...
            if ((0, frenetic_1.toLowerFast)(switchPair.value) !== 'true' && (0, frenetic_1.toLowerFast)(switchPair.value) !== 'false') {
                warnScript(checker.warnings, eventValue.line, 'bad_switch_value', `'${switchPair.key}' switch invalid: must be 'true' or 'false'.`);
            }
        }
            ...
            if (!parsesAsDouble(switchPair.value)) {
                warnScript(checker.warnings, eventValue.line, 'bad_switch_value', `'${switchPair.key}' switch invalid: must be a decimal number.`);
            }
        }
        else if (switchPair.key === 'in' || switchPair.key === 'location_flagged') {
            if (!matchedEvent.hasLocation) {
                warnScript(checker.warnings, eventValue.line, 'unknown_switch', `'${switchPair.key}' switch is only supported on events that have a known location.`);
            }
        }
        else if (switchPair.key === 'flagged' || switchPair.key === 'permission') {
            if (matchedEvent.player.trim().length === 0) {
                warnScript(checker.warnings, eventValue.line, 'unknown_switch', `'${switchPair.key}' switch is only supported on events that have a linked player.`);
            }
        }
        else if (switchPair.key === 'assigned') {
            if (matchedEvent.npc.trim().length === 0) {
                warnScript(checker.warnings, eventValue.line, 'unknown_switch', `'${switchPair.key}' switch is only supported on events that have a linked NPC.`);
            }
        }
        else {
